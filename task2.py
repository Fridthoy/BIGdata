...
def task23(postrdd):
    ...
    cleanId= postrdd.filter(lambda x: x != header).map(lambda x: x[6]).filter(lambda x: x != "-1")
...
def task25(userRdd):
    ...
    teller = downAndUp.map(lambda element: (element[0]- meanUp)*(element[1] - meanDown)).sum()
...
def task26(commentRdd):
    ...
    newCom = commentRdd.map(lambda x: x[4]).map(lambda x:[x,1]).reduceByKey(lambda a, b: a+b)
